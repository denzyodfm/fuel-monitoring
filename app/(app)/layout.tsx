...
import {BarChart3,BookOpen,Car,ClipboardList,Cog,LayoutDashboard,LogOut,Menu,Receipt,ShieldCheck,Users} from "lucide-react";

// Settings is open to every role: it hosts Change password. The sections inside it
// (branding, users, audit log) are gated on ADMINISTRATOR by the page itself.
const navLinks=[["/dashboard","Dashboard",LayoutDashboard],["/transactions","Transactions",Receipt],["/companies","Companies",ClipboardList],["/vehicles","Vehicles",Car],["/operators","Operators",Users],["/purposes","Purposes",Menu],["/insurance","Insurance",ShieldCheck],["/reports","Reports",BarChart3],["/settings","Settings",Cog]] as const;

export default async function AppLayout({children}:{children:React.ReactNode}){
  const [session,branding]=await Promise.all([requireSession(),getBranding()]);
  ...
  return <div className="min-h-screen flex brand-shell">
    <aside className="sidebar w-64 bg-slate-950 text-slate-300 p-4 fixed inset-y-0 overflow-y-auto">
      ...
      <nav className="space-y-1">{navLinks.map(([href,label,Icon])=><Link key={href} href={href} className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-800"><Icon size={17}/>{label}</Link>)}<a href="/manual.html" target="_blank" rel="noreferrer" className="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-slate-800"><BookOpen size={17}/>User manual</a></nav>
    </aside>
