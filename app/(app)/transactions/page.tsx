...
export default async function Page({searchParams}:{searchParams:Promise<Q>}){
  ...
  const vehicleOptions=vehicles.map(item=>({value:item.id,label:vehicleLabel(item.plateNumber,item.assetName),fuelType:item.fuelType??""})),operatorOptions=operators.map(item=>({value:item.id,label:item.fullName})),companyOptions=companies.map(item=>({value:item.id,label:`${item.code} - ${item.name}`})),purposeOptions=purposes.map(item=>({value:item.id,label:item.name}));
